...
void max_n_dot_f_of_positive_scalar(
    ...
    const Element<Dim>& element, const std::array<double, Dim>& element_size,
    const TimeDelta& dt) noexcept {
  const double mean_scalar = mean_value(get(scalar), mesh);
  ASSERT(mean_scalar > 0.0,
         "scalar already avg-negative :(\n"
         "element id = "
             << element.id());
...
struct MaxNormalFluxMassDensityCompute : db::ComputeTag,
  ...
  using argument_tags =
      tmpl::list<MassDensityCons, domain::Tags::Mesh<Dim>,
                 domain::Tags::Element<Dim>, domain::Tags::SizeOfElement<Dim>,
                 ::Tags::TimeStep>;

  static constexpr void (*function)(
      gsl::not_null<Scalar<DataVector>*>, const Scalar<DataVector>&,
      const Mesh<Dim>&, const Element<Dim>&, const std::array<double, Dim>&,
