...
template <size_t Dim>
void test_char_transformation_helpers() {
  ...
  const tnsr::I<double, Dim> mean_momentum_density =
      [&momentum_density, &mesh ]() noexcept {
  ...
  const auto right_and_left = NewtonianEuler::compute_eigenvectors<Dim>(
  ...
  Limiters::NewtonianEulerWeno_detail::char_tensors_from_cons_tensors(
...
SPECTRE_TEST_CASE("Unit.Evolution.Systems.NewtonianEuler.Limiters.Helpers",
                  "[Unit][Evolution]") {
